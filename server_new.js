...
// Get the codestrings from the codestring server:
    ...
    const sorted_q_comm = q_comm.sort(function (a, b) {
        return a[0].q_ind - b[0].q_ind
    });
    const sorted_m_comm = m_comm.sort(function (a, b) {
        return a[0].m_ind - b[0].m_ind
    });
...
function load_current_query_diat_str(q_diat_url) {
    load_file(q_diat_url, get_diat_str);
}
...
function load_mei(url) {
    load_file(url, get_mei);
}
