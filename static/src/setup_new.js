...
test_codestring(test_id);

const request = require('request');
function test_codestring(id) {
console.log("Seeking codestring for "+id)
	var codestring_found = "";
	var options = {
	    url: "http://localhost:8500/api/id",
	    json: true,
	    body: {
		   id: id    }
	};
	request.post(options, function(error, response, body) {
	  codestring_found = body;
	  if(codestring_found) console.log("Found codestring: "+ codestring_found);
	  else console.log("No codestring found!")
	});
}

/// Following code is for searching multiple data-segment servers and combining results

function get_data_segments(){
	var data_segs = [];
...
// Multiple remote search function. Repeats search for each data-segment in turn on its own port.
// NB Results must be dynamically concatenated in the browser - see show_multi_results
function multi_search(id, jaccard, num_results, ngram_search, segments) {
    search_data = JSON.stringify({ id, jaccard, num_results, threshold, ngram_search, segments});
    console.log(search_data);
    $('#results_table').html('<tr><td><img src="img/ajax-loader.gif" alt="search in progress"></td></tr>'); 
    clear_multi_results();
    for(var i=0;i<segments.length;i++) {
	    var port = 8001+segments[i];
	    $.ajax({
...
function clear_multi_results() {
	document.getElementById("result_table_col").innerHTML = "";
	$('#results_table').html('<tr><td><img src="img/ajax-loader.gif" alt="search in progress"></td></tr>'); 
